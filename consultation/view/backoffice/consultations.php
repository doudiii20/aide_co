...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id_rendezVous) {
        ...
        if ($controller->updateRendezVous($rendezVous)) {
            ...
            if ($sendEmail) {
                $consultationData = $rendezVous->toArray();
                if ($emailService->sendConsultationConfirmation(
                    $_POST['email_universitaire'],
                    $_POST['nom_complet'],
                    $consultationData
                )) {
                    ...
                    $message .= " (Note: L'envoi de l'email a échoué, mais le rendez-vous a été enregistré)";
                }
            }
        } else {
            ...
            <form method="POST" id="data-form" novalidate>
                ...
                <div class="form-group" style="background-color: rgba(211, 47, 47, 0.1); padding: 20px; border-radius: 8px; border-left: 4px solid var(--primary-red); margin: 20px 0;">
                    ...
                    <p style="margin-top: 10px; color: var(--text-muted); font-size: 13px; margin-left: 30px;">
                        ...
                        Un email de confirmation avec les détails du rendez-vous sera envoyé au patient.
                    </p>
                </div>
